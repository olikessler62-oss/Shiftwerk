-- Superadmin-Testszenario „Friseur-Salon / Zentrale“: Baseline-Cleanup vor Neuaufbau.

create or replace function public.prepare_friseur_salon_scenario(p_organization_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  set archived_at = now()
  where organization_id = p_organization_id
    and lower(trim(name)) <> lower('Friseur')
    and archived_at is null;
...
grant execute on function public.prepare_friseur_salon_scenario(uuid) to service_role;
