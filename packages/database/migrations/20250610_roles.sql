-- Rollen-Tabelle + profiles.role_id (einmalig in Supabase ausführen)
...
-- Standard-Rollen je Organisation
...
-- Alte Enum-Spalte role → role_id (nur wenn role noch existiert)
do $$
begin
  if exists (
    select 1
...
-- Verbleibende NULLs auf basic-Rolle setzen
update public.profiles p
set role_id = r.id
from public.roles r
where r.organization_id = p.organization_id
...
-- WICHTIG: Rückgabetyp ändert sich (role → permission_level) — DROP nötig
...
create policy "roles_write_manager"
  ...
  using (
    organization_id in (
      select cp.organization_id from public.current_profile() cp
      where cp.permission_level in ('admin', 'manager')
    )
  )
  with check (
...
-- RLS-Policies: role → permission_level
