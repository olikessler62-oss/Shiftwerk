-- Dev/test reset: clears shift-related operational data for one organization.
-- Preserves: profiles, absences, locations, areas, templates, etc.
-- Resets profile_recurring_availability to 07:00–22:00 (Mo–So) for all profiles.
-- Resets service hours for Restaurant, Küche, and Bar (Thu closed).
-- Resets profile qualifications: Kellner/in (all), Koch/Köchin (first 7), Spülkraft (7–13), Barista (last 7).
