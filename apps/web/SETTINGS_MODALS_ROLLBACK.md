...
1. In `apps/web/src/lib/settings-modal-config.ts` setzen:
   ...
   ```
2. Optional: `SettingsModalsLayer` aus `shift-planner.tsx` und `SettingsModalsAppShellFallback` aus `app-shell.tsx` entfernen (bei `false` reicht Schritt 1 — Sidebar und Schließen-Logik leiten wieder nach `/dashboard`).
...
- `src/components/planning/shift-planner.tsx` — Layer auf Planung
