...
import {
  ...
  areaCalendarOvernightShiftDurationMinutes,
...
describe("resolveAreaCalendarAreaServiceDayTimeline", () => {
  it("caps overnight service entries at 23:59 for cell division", () => {
    const timeline = resolveAreaCalendarAreaServiceDayTimeline(
      TUESDAY_HOURS,
      AREA_ID,
      "2026-06-16"
    );
...
describe("overnight width service spans", () => {
  ...
  it("computes overnight shift duration across midnight", () => {
    expect(areaCalendarOvernightShiftDurationMinutes("22:00", "04:00")).toBe(6 * 60);
    expect(areaCalendarOvernightShiftDurationMinutes("22:00", "06:00")).toBe(8 * 60);
  });

  it("uses midnight to latest same-day service end on the follow-up day", () => {
    const timeline = resolveAreaCalendarOvernightEndDayTimeline(
      WEDNESDAY_HOURS,
      AREA_ID,
      "2026-06-17"
    );
  ...
  it("matches the documented 8h shift on 26h service example", () => {
    const startSpan = areaCalendarStartDayServiceSpanMinutesForOvernightWidth(
      [
        {
          id: "d1a",
          location_area_id: AREA_ID,
    ...
    const endSpan = areaCalendarEndDayServiceSpanMinutesForOvernightWidth(
      [
        {
          id: "d2a",
          location_area_id: AREA_ID,
    ...
    const shiftMin = areaCalendarOvernightShiftDurationMinutes("22:00", "06:00");
