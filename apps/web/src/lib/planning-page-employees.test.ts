...
import { resolvePlanningEmployeesForShifts } from "./planning-page-employees";
...
describe("resolvePlanningEmployeesForShifts", () => {
  it("returns active employees unchanged when all shift assignees are included", async () => {
    ...
    const result = await resolvePlanningEmployeesForShifts(
      active,
      [{ employee_id: "a" }],
      vi.fn(),
      "org-1"
