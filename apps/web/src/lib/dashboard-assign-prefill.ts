...
import {
  ...
  employeeMatchesAreaCalendarShiftWindow,
...
function employeeEligibleForPlanningDemand(
  ...
  const weekday = profileAvailabilityWeekdayFromAreaCalendarDate(dateISO);
  if (
    !employeeHasRecurringAvailabilityOnWeekday(
      employee.id,
  ...
  return employeeMatchesAreaCalendarShiftWindow(
    employee.id,
    recurringAvailability,
    weekday,
    startTime,
 ...
 */
export function resolveDashboardAssignPrefillFromOpenDemand(input: {
  employeeId: string;
  dateISO: string;
  areaId: string;
  staffingEntries: readonly TagAreaHeaderStaffingEntry[];
