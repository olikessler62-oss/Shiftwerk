...
export const DASHBOARD_DAY_DRILLDOWN_WEEK_VALUE = "__week__";
...
export function DashboardDayDrilldownDayCombobox({
  days,
  value,
  valueLabel = null,
  onChange,
  ...
  const options = useMemo(() => {
    const weekOption: DayOption[] = showWeekOption
      ? [
          {
            value: DASHBOARD_DAY_DRILLDOWN_WEEK_VALUE,
            label: t("dashboard.areaScopeWeek"),
          },
        ]
      : [];
    return [...weekOption, ...days];
  }, [days, showWeekOption, t]);

  const selected =
    options.find((option) => option.value === value) ??
    (value && value !== DASHBOARD_DAY_DRILLDOWN_WEEK_VALUE && valueLabel
  ...
  const dropdownPanel =
    open && !disabled && options.length > 0 && dropdownStyle ? (
      <ul
        ...
        id={listboxId}
        role="listbox"
        aria-label={t("dashboard.dayDrilldownDayComboboxAriaLabel")}
        style={dropdownStyle}
        className={cn(
